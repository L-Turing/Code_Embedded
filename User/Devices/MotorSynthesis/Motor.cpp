...
void Motor_Class::SetPitch(float pitch_angle_set) {
  PID_Calc(this->pid_position, INS.Roll, pitch_angle_set);
  PID_Calc(this->pid_speed, INS.Gyro[1] * 57.29578f, this->pid_position->output);
  this->output = -this->pid_speed->output;
}

void Motor_Class::SetYaw(float yaw_angle_set) {
  PID_Calc(this->pid_position, INS.YawTotalAngle, yaw_angle_set);
  PID_Calc(this->pid_speed, INS.Gyro[2], this->pid_position->output);
  this->output = this->pid_speed->output;
}

void Motor_Class::SetAmmunition(float ammunition_set, motor_control_modes m) {
  if (m == motor_control_modes::Position_Mode) {
    PID_Calc(this->pid_position, this->accumlate_rad_out * 57.29578f, ammunition_set);
    PID_Calc(this->pid_speed, this->speed_rpm_rotor, this->pid_position->output);
  }
  else if (m == motor_control_modes::Speed_Mode) {
    PID_Calc(this->pid_speed, this->speed_rpm_rotor, ammunition_set);
  }
  this->output = this->pid_speed->output;
}

void Motor_Class::SetFriWheel(float fri_rpm) {
  this->output = PID_Calc(this->pid_speed, this->speed_rpm_rotor, fri_rpm);
}
...
void Motor_Class::Update_Info() {
  ...
  if (this->motor_type == motor_types::GM6020 || this->motor_type == motor_types::M2006 ||
    this->motor_type == motor_types::M3508)
  {
    ...
    this->accumlate_rad_rotor =
      (float)((this->circle_number_rotor) * 8192 + this->last_ecd) /
      8192.0f * 2 * PI;  // 转子累计角度 rad
    this->accumlate_rad_out = this->accumlate_rad_rotor / this->reduction_ratio;  // 输出轴累计角度 rad
  }
  else if (this->motor_type == motor_types::DaMiao4310 || this->motor_type == motor_types::DaMiao3507)
  {
...
void DJIMotor_Class::DJIFeedback(uint8_t* canbuf_receive) {
